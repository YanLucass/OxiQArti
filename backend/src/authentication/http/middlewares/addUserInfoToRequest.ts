...
// Defines the expected type for the properties of the JWT payload(from decode)
type jwtPayloadProps = {
   sub: string;
};

// Middleware to add user information(user_id) to the request based on the JWT token
export const addUserInfoToRequest = (req: Request, res: Response, next: NextFunction) => {
   ...
   try {
      ...
      // Extract the 'sub' property (where is user_id) from the token payload
